...
Ein Autor ist als \gls{tp} klassifiziert worden, falls er richtig zugeordnet wurde.
Falls der Autor mehrfach in der Git-Autorenliste vorkommt, wurde der Autor als \gls{tp} klassifiziert unabhängig davon, mit welchem von den doppelten Einträgen der Autor zugeordnet wurde.
...
Beispielsweise werden in \autoref{fig:common_authors_pypi_cff} von 100 betrachteten Top-Git-Autoren gemessen nach Commits nahezu 0 \% als Python Maintainer genannt, jedoch werden knapp 10 \% unter dem \gls{cff} \emph{preferred-citation} Feld angegeben.
Dabei muss berücksichtigt werden, dass unterschiedliche Quellen unterschiedlich häufig vorkommen.
Beispielsweise haben weniger Pakete Autoren unter dem \gls{cff} Feld \emph{preferred-citation} angegeben als direkte \gls{cff} Autoren, da diese zwingend angegeben werden müssen.
...
\autoref{fig:citation_counts} zeigt für die Quellen \gls{cff}, \gls{cff} \emph{preferred-citation} und \hologo{BibTeX} den angegebenen Typ der Zitation.
...
\begin{figure}
    ...
    \begin{subfigure}{.5\textwidth}
        ...
        \caption{\gls{cff} \emph{preferred-citation}}
...
Die \autoref{fig:added_removed_authors} zeigt für alle fünf Listen, wie viele Autoren in den einzelnen Quellen hinzugefügt und entfernt wurden.
...
\begin{table}
    \begin{tabularx}{\textwidth}{XR{1.42cm}R{1.2cm}R{.93cm}R{2.2cm}R{2.6cm}}
        ...
        \gls{cff} preferred citation & \infty &        & 512    & 512    & 140 \\
...
\begin{table}
    \begin{tabularx}{\textwidth}{XR{1.42cm}R{1.2cm}R{.93cm}R{2.2cm}R{2.6cm}}
        ...
        \gls{cff} preferred citation & 1021 &      & 676  & 559  & 723 \\
