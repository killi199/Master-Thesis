...
% TODO Auf Auffälligkeiten eingehen und die anekdotisch untermauern
% TODO auf Anhang eingehen
% TODO auf Besonderheiten eingehen
% TODO ordentlich beschreiben was es mit den nicht abgeglichenen Autoren auf sich hat also einmal inklusive einmal exklusive
%In der \autoref{tab:similarity_between_authors} wird für jede Liste angegeben, wie ähnlich die Autoren in den Paketen innerhalb der Quellen sind.
...
%Falls ein Autor nicht abgeglichen werden konnte, wird in der Tabelle unterschieden, ob dieser mit in die Berechnung einfließt oder nicht.
%Falls er einfließt wird, führt das zu schlechteren Ergebnissen, da der Autor in keiner anderen Quelle vorhanden sein kann, da er nicht abgeglichen wurde.

%In der Tabelle lässt sich beispielsweise ablesen, dass die Autoren der Pakete in der \gls{cran} Liste zu 42.17 \% in den Quellen übereinstimmen.
%Dabei wird der Durchschnitt über alle Pakete gebildet. 
...
%    \begin{tabular}{c|c|c|c|c|c}
%        \toprule
%         & \textbf{\gls{cran}} & \textbf{\gls{pypi}} & \textbf{\gls{cff}} & \textbf{\gls{pypi} \gls{cff}} & \textbf{\gls{cran} \gls{cff}} \\ \midrule
%        Inkl. & 42.17 \% & 25.20 \% & 20.76 \% & 25.15 \% & 45.66 \% \\
%        Exkl. & 45.63 \% & 30.38 \% & 24.95 \% & 33.95 \% & 57.70 \% \\
%        \bottomrule
%    \end{tabular}
%    \caption{}
%    \label{tab:similarity_between_authors}
%    \small
%    \raggedright
%    Die Tabelle zeigt in Prozent für die unterschiedlichen Listen, wie ähnlich die Autoren innerhalb der Quellen in den Paketen angegeben sind. Dabei wird unterschieden, ob Autoren, welche nicht abgeglichen werden konnten, berücksichtigt werden oder nicht.
%\end{table}

\begin{figure}
    \begin{subfigure}{.5\textwidth}
        \centering
        \includesvg[width=.95\linewidth,inkscapelatex=false]{bilder/similarity/similarity_with_non_matches_pypi_cff.svg}
        \caption{\gls{pypi} \gls{cff} inklusive nicht abgeglichener Autoren}
        \label{fig:similarity_with_non_matches_pypi_cff}
    \end{subfigure}%
    \begin{subfigure}{.5\textwidth}
        \centering
        \includesvg[width=.95\linewidth,inkscapelatex=false]{bilder/similarity/similarity_without_non_matches_pypi_cff.svg}
        \caption{\gls{pypi} \gls{cff} exklusive nicht abgeglichener Autoren}
        \label{fig:similarity_without_non_matches_pypi_cff}
    \end{subfigure}
    \begin{subfigure}{.5\textwidth}
        \centering
        \includesvg[width=.95\linewidth,inkscapelatex=false]{bilder/similarity/similarity_with_non_matches_cran_cff.svg}
        \caption{\gls{cran} \gls{cff} inklusive nicht abgeglichener Autoren}
        \label{fig:similarity_with_non_matches_cran_cff}
    \end{subfigure}%
    \begin{subfigure}{.5\textwidth}
        \centering
        \includesvg[width=.95\linewidth,inkscapelatex=false]{bilder/similarity/similarity_without_non_matches_cran_cff.svg}
        \caption{\gls{cran} \gls{cff} exklusive nicht abgeglichener Autoren}
        \label{fig:similarity_without_non_matches_cran_cff}
    \end{subfigure}
