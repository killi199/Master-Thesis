...
% TODO Unbedingt Vergleich von nur einem Buchstaben erwähnen referenziere das!
% TODO auf schlechte F1 Scores von README und beschreibung eingehen und auch sagen warum da so viele TN sind (Die NER erkennt viel müll)
% TODO darauf eingehen, warum in Python Quellen so viele nicht Personen als Autoren genannt werden
% TODO begründen warum bibtex so das schlechteste ist vom F1 Score ist -> so wenig autoren betrachtet falls mehr betrachtet werden würde wäre es besser (Hier werden vier Autoren insgesamt betrachtet, was durch die geringe Anzahl an \hologo{BibTeX}-Dateien in den untersuchten Listen und der manuellen Betrachtung ausschließlich der ersten beiden Autoren in den Paketen begründet ist.)
