...
async def process_and_save(dataframe, git_contributors_df, package_name, filename):
...
async def process_general_package(owner: str, repo: str, git_contributors_df, package_name: str):
    cff_df = functions.get_cff_authors(owner, repo)
    await process_and_save(cff_df, git_contributors_df, package_name, 'cff_authors')

    cff_preferred_df = functions.get_cff_preferred_citation_authors(owner, repo)
    await process_and_save(cff_preferred_df, git_contributors_df, package_name, 'cff_preferred_citation_authors')

    bib_df = functions.get_bib_authors(owner, repo)
    await process_and_save(bib_df, git_contributors_df, package_name, 'bib_authors')

    readme_df = functions.get_readme_authors(owner, repo)
    await process_and_save(readme_df, git_contributors_df, package_name, 'readme_authors')
...
async def process_pypi_package(package):
    ...
    try:
        ...
        await process_and_save(pypi_maintainers_df, git_contributors_df, package_name, 'pypi_maintainers')

        python_authors_df = functions.get_python_authors(pypi_data)
        await process_and_save(python_authors_df, git_contributors_df, package_name, 'python_authors')

        python_maintainers_df = functions.get_python_maintainers(pypi_data)
        await process_and_save(python_maintainers_df, git_contributors_df, package_name, 'python_maintainers')

        description = pypi_data['info']['description']
        description_df = functions.get_description_authors(description)
        await process_and_save(description_df, git_contributors_df, package_name, 'description_authors')

        await process_general_package(owner, repo, git_contributors_df, package_name)
...
async def process_cran_package(package):
    ...
    try:
        ...
        await process_and_save(cran_authors_df, git_contributors_df, package_name, 'cran_authors')

        cran_maintainers_df = functions.get_cran_maintainers(cran_data)
        await process_and_save(cran_maintainers_df, git_contributors_df, package_name, 'cran_maintainers')
